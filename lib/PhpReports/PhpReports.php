...
class PhpReports
{
    ...
    public static function init($config = 'config/config.php')
    {
        ...
        spl_autoload_register(['PhpReports', 'loader'], true, true);
    ...
    public static function getReportListJSON($reports = null)
    {
        ...
        foreach ($reports as $report) {
            if ($report['is_dir'] && $report['children']) {
                ...
                if ((isset($report['stop']) && $report['stop']) || isset($report['Caution'])
                    || isset($report['warning'])) {
                    continue;
                }
                if (!isset($report['url'])) {
                    continue;
    ...
    public static function emailReport()
    {
        ...
        if (!isset(PhpReports::$config['mail_settings']['enabled'])
            || !PhpReports::$config['mail_settings']['enabled']) {
        ...
        $email_html =
            "<p>$body</p>$table<p>View the report online at <a href=\"".htmlentities($link)."\">".htmlentities($link)
            ."</a></p>";
    ...
    public static function buildLoaderCache()
    {
        ...
        self::load('classes', ['classes/local']);
        self::load('lib');
    }

    public static function load($dir, $skip = [])
    {
        $files = glob($dir.'/*.php');
        ...
        foreach ($dirs as $dir2) {
            ...
            if (in_array(basename($dir2), ['tests', 'test', 'example', 'examples', 'bin'])) {
                continue;
            }
